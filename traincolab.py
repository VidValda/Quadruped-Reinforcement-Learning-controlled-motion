...
Original file is located at
    https://colab.research.google.com/drive/1_ibWynabm7FxRfXP6WZkSQyEX_YRiqix
"""
...
# %tensorboard --logdir logs/spot_genesis_sb3
...
def get_cfgs():
    """
    Configuration for Spot, adapted from Go2 config but with Spot physics/joints.
    Updated naming convention to match mujoco_menagerie (fl.hx, etc.)
    """
    env_cfg = {
        "num_actions": 12,
        # Robot Path from robot_descriptions
        "robot_path": SPOT_XML_PATH,
        # Spot Joint Names (Matches mujoco_menagerie/boston_dynamics_spot/spot.xml)
        "dof_names": [
            "fl_hx", "fl_hy", "fl_kn",
            "fr_hx", "fr_hy", "fr_kn",
            "hl_hx", "hl_hy", "hl_kn",
        ...
        # Default Angles (Approximate for Spot standing)
        ...
        "kp": 50.0, # Spot is heavier/stiffer than Go2, slightly higher gains
        "kd": 1.0,
        "termination_if_roll_greater_than": 0.4, # rad (~23 deg)
        "termination_if_pitch_greater_than": 0.4,
        "base_init_pos": [0.0, 0.0, 0.55], # Spot is taller (approx 50-60cm standing)
    ...
    reward_cfg = {
        "tracking_sigma": 0.25,
        "base_height_target": 0.50, # Spot target height
    ...
    command_cfg = {
        "num_commands": 5, # [lin_vel_x, lin_vel_y, ang_vel, target_height, jump_trigger]
        "lin_vel_x_range": [-1.0, 2.0],
        "lin_vel_y_range": [-0.5, 0.5],
        "ang_vel_range": [-0.8, 0.8],
        "height_range": [0.4, 0.6], # Spot height range
        "jump_range": [0.6, 0.8],    # Target Jump Height
    }

    return env_cfg, obs_cfg, reward_cfg, command_cfg

def main():
    # --- Colab Arguments Setup ---
    # Since argparse is difficult to use in Colab notebooks, we use a simple class.
    class Args:
        exp_name = "spot_genesis_sb3"
        num_envs = 4096  # Adjust based on Colab GPU VRAM (e.g., 1024 for T4, 4096 for A100)
    ...
    # --- Genesis Init ---
    try:
        gs.init(logging_level="warning", backend=gs.constants.backend.gpu)
    except gs.GenesisException as e:
        print(f"Genesis initialization skipped: {e}")
    ...
    # --- Create Environment ---
    print(f"Creating Spot Environment with {args.num_envs} instances...")
    env_base = SpotGenesisEnv(
        ...
        show_viewer=False, # Disable viewer for training speed
        device="cuda:0"
    )
    ...
    # --- Setup PPO ---
    # Note: We use MlpPolicy. The observations are 1D vectors.
    model = PPO(
        "MlpPolicy",
        env,
        verbose=1,
        learning_rate=3e-4,
        n_steps=24, # Keeps consistent with Go2 config (short rollouts, many envs)
        batch_size=4096, # Large batch size due to many envs
        n_epochs=5,
        gamma=0.99,
        gae_lambda=0.95,
        clip_range=0.2,
        tensorboard_log=log_dir,
        device="cuda", # PPO training on GPU
    )
