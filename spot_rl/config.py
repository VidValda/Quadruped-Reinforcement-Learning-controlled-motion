...
class CommandConfig:
    lin_vel_x_range: tuple[float, float] = (-0.2, 0.5)
    lin_vel_y_range: tuple[float, float] = (-0.2, 0.2)
    ang_vel_range: tuple[float, float] = (-0.2, 0.2)
...
@dataclass(frozen=True)
class PathConfig:
    model_path: Path = ROOT_DIR / "models" / "ppo_spot_v28.zip"
    stats_path: Path = ROOT_DIR / "stats" / "vec_normalize_stats_v28.pkl"
